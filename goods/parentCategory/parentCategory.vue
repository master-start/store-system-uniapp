	...
	methods: {
		...
		changeMethod(data, button, index){
			...
			if(actionTitle == "编辑"){
				...
				//分类
				this.getParentList();
				this.GoodsCategoryId = data.Parent_id;
				this.$refs.pop.show();
			}
			...
			app.request().postRequest(
				...
				if (res.code === 1) {
					...
					productionData.map((g,index)=>{
						...
						console.log("gid是",g.id);
						console.log("上级id是",Parent_id);
						
						if(g.id == Parent_id){
							that.GoodsCategoryChangeId = parseInt(index) + 1;
							console.log("设置的分类id是",index);
						}
					})
					console.log("取到的数据为",GoodsCategoryArray);
					that.GoodsCategoryArray = GoodsCategoryArray;
					console.log("取到的id数据为",GoodsCategoryIdArray);
					that.GoodsCategoryIdArray = GoodsCategoryIdArray;
		...
		getDataList(){
			...
			app.request().postRequest(
				app.getNetAddresss('api/GoodsCategory/parentList'),
				{
					UserMobile: uni.getStorageSync('mobile'),
					Parent_id:Parent_id
				},

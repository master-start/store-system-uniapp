		...
		<view class="content">
			<click-tab :tabs="tabs" @onClickChange="onTabChange">
						...
						<view class="ContentBackground">
							<form @submit="addFormSubmit">
								<view class="create-content">
									...
									<view class="cu-form-group section">
										<view class="title">品牌描述：</view>
										<input type="text" name="Desc" placeholder="请输入品牌描述" />
									</view>
									
									<view class="cu-form-group section">
										<view class="title">品牌排序：</view>
				...
				<view slot="content1">
					<view :class="content1">
						<view class="ContentBackground">
							<goods-brand-list :list="setDataList" :button="buttonList" :border="true" @click="clickMethod" @change="changeMethod"></goods-brand-list>
						</view>
									...
									<view class="ContentBackground">
										<form @submit="editFormSubmit" :id="setDataInfo.id">
											<view class="create-content">
												...
												<view class="cu-form-group section">
													<view class="title">品牌名称：</view>
													<input type="text" name="Name" :value="setDataInfo.Name" placeholder="请输入品牌名称" />
												</view>
												...
												<view class="cu-form-group section">
													<view class="title">品牌描述：</view>
													<input type="text" name="Desc" :value="setDataInfo.Desc" placeholder="请输入品牌描述" />
												</view>
...
//列表数据
import goodsBrandList from '@/components/mark-slide-list/goods-brand-list.vue';
//弹窗
import pop from "@/components/ming-pop/ming-pop.vue"
const app = getApp({ allowDefault: true });
const globalData = app.globalData;
export default {
	components: { clickTab,goodsBrandList,pop,tmUpload },
	data() {
		return {
			//照片上传
			upLoadUrl:'',
	...
	methods: {
			...
			if(actionTitle == "停用"){
				...
				uni.showModal({
					...
					success:function(res){
						if (res.confirm) {
							console.log('用户点击确定');
							app.request().postRequest(app.getNetAddresss('api/GoodsBrand/modifyStatus'), {
								'id':data.id,
								'Is_Hide':1
							}, {
		...
		editFormSubmit(input){
			...
			app.request().postRequest(
				app.getNetAddresss('api/GoodsBrand/edit'),
				{
					BrandId: uni.getStorageSync('BrandId'),
					Name: Name,
					id: id,
		...
		addFormSubmit(input){
			...
			let FirstImg = that.FirstImg;
			app.request().postRequest(
				app.getNetAddresss('api/GoodsBrand/create'),
				{
					BrandId: uni.getStorageSync('BrandId'),
					Name: Name,
					Logo: FirstImg,

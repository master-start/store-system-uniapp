<template>
	<view class="hx-store" @touchstart="touchStart">
		...
		<view class="main" :style="{height: mainHeight}">
			<view class="tabs-box" style="top:0rpx;">
				<view class="" style="height:100%;">
					<view class="hx-tabs">
						<view class="hx-tabs-item" v-for="(item,i) in tabs" 
						 :key="i" 
			...
			>
					...
					<view class="scroll-items">
						<view class="category-list">
							...
							<scroll-view :scroll-top="menuScrollTop" :scroll-y="goodsBoxScroll" class="left">
								<view v-for="item in categoryList" 
								    :key="item.id" class="row"
									:class="{active: item.id == menuCurrentId}" 
							...
							<scroll-view scroll-with-animation scroll-y="true" class="right"  @scroll="asideScroll" :scroll-top="tabScrollTop" >
									<view class="goodsListBox">
										<view class="category">
											<!-- 分类名称 -->
											<view class="s-item">{{categorytitle}}</view>
											<!-- 商品列表 -->
											<view class="list">
												<block v-if="goodsList && goodsList[0]">
													<view class="box" v-for="(rowData,i) in goodsList" :key="rowData.id">
														...
														<view class="m-store-item">
															<view class="m-img" @click="hrefGoodsInfo(rowData.id)">
																<image style="width: 100%;height: 100%;" :src="rowData.FirstImg" mode="aspectFit"></image>
															</view>
															<view class="m-text">
															...
															<view class="m-Description" @click="hrefGoodsInfo(rowData.id)">
																{{rowData.Description}}
															</view>
														
															<block v-if="rowData.has_many_sku != [] && rowData.has_many_sku[0]">
																<!-- sku的商品 -->
																<view class="m-price-box">
																	<view class="symbol" v-if="rowData.has_many_sku[0].Price">￥</view>
																	<view class="m-price" v-if="rowData.has_many_sku[0].Price">{{rowData.has_many_sku[0].Price}}</view>
																	<view class="m-old-Price" v-if="rowData.has_many_sku[0].MarketPrice">
																		<text>￥{{rowData.has_many_sku[0].MarketPrice}}</text>
																	</view>
																</view>
															</block>
															...
															<block v-else>
																...
																<view class="m-price-box">
																	...
																	<view class="m-old-Price" v-if="rowData.MarketPrice">
																		<text>￥{{rowData.MarketPrice}}</text>
																	</view>
																</view>
															</block>
												...
												<block v-else>
													<view class="none-content">
														<view><image src="../../static/img/none.png"></image></view>
				...
				<!-- 评价 -->
				<swiper-item class="swiper-item">
					<scroll-view scroll-y style="height: 100%;width: 100%;background-color: #ffffff;" @scroll="asideScroll" :scroll-y="true" >
				...
				<swiper-item class="swiper-item">
					<view class="scroll-items business-box">
						...
						<view class="info-list" style="padding-top: 30rpx; padding-bottom: 20rpx; padding-left: 20rpx; text-align: center;" v-if="storeData.StoreImg">
							<image :src="storeData.StoreImg" style="width: 550rpx; text-align: center; border-radius: 20rpx;"></image>
						</view>
						
						<view class="info-list hx-mt-15 " v-if="storeData.StoreName">
							<view class="info-list-container hx-bb">
								<i class="hxicon-time"></i>
								<text>门店名称：{{ storeData.StoreName }}</text>
							</view>
						</view>
						
						<view class="info-list hx-mt-15" v-if="storeData.StoreAddress">
							<view class="info-list-container">
								<i class="hxicon-location"></i>
								<text>{{ storeData.StoreAddress }}</text>
							</view>
						</view>
						
						
						<view class=" info-list" v-if="storeData.StoreMobile">
							<view class="info-list-container" @click="goCall(storeData.StoreMobile)">
								<i class="hxicon-phone"></i>
								<text style="flex: 1;">商家电话：{{ storeData.StoreMobile }}</text>
								<view class="right">
						...
						<view class="info-list hx-mt-15 " v-if="storeData.store_industry_id_max.IndustryName">
							<view class="info-list-container hx-bb">
								<i class="hxicon-redpacket"></i>
								<text>所在行业：{{ storeData.store_industry_id_max.IndustryName }} -- {{ storeData.store_industry_id_min.IndustryName }}</text>
							</view>
						</view>
		...
		<uni-popup ref="popup" type="center" @change="popupChange">
			<view class="form-main" v-if="currentGoodsData.Title">
				<view class="form-main_ctn" @click.stop.prevent="mpClear" >
					<view class="godos_tit"><text>{{currentGoodsData.Title}}</text></view>
					
					<view class="gg_tit">
						<text>{{currentGoodsData.has_many_sku.Title}}</text>
					</view>
					<view class="gg_box">
						<block v-for="has_many_sku in currentGoodsData.has_many_sku" :key="has_many_sku.id">
							<view class="item" :class="{'active': has_many_sku.select}" @click="selectGoodsForm(currentGoodsData,has_many_sku)">{{has_many_sku.Title}}</view>
						</block>
					</view>
					...
					<view class="select_gg">
						...
						<view class="select_gg_box">
							<block v-for="has_many_sku in currentGoodsData.has_many_sku" :key="has_many_sku.id" v-if="has_many_sku.select == true">
								<view class="gg-item">
									<text>{{has_many_sku.Title}} </text>
					...
					<view class="bottom">
						<view class="Price_box">
							<text>￥</text>
							<block v-for="(has_many_sku,i) in currentGoodsData.has_many_sku" :key="i" v-if="has_many_sku.select == true">
								<text class="Price">{{has_many_sku.Price}}</text>
							</block>
						</view>
...
<script>
	...
	export default {
		...
		onReady() {
			...
			//屏幕高度 - 头部导航高度 - 状态栏高度 - 分页高度 - 差距
			this.mainHeight = sysInfo.screenHeight - 43 - statusBarHeight - 40 - 6  + 'px';
		...
		methods: {
			...
			getMarkeing(){
				app.request().postRequest(
					app.getNetAddresss('api/Marketing/getData'),
					{
			...
			getStoreInfo(){
				app.request().postRequest(
					app.getNetAddresss('api/store/getStoreInfo'),
					{
						UserMobile: uni.getStorageSync('mobile'),
					},
					{
					...
					if (res.code === 1) {
						console.log("获取到的门店数据为",res.result);
			...
			//获取产品列表
			getGoodsList(){
				var firstcategoryid = this.firstcategoryid;
				app.request().postRequest(
					app.getNetAddresss('api/Goods/getList'),
					{
						UserMobile: uni.getStorageSync('mobile'),
						GoodsCategoryId:firstcategoryid,
						data_type:1
					},
					{
						'content-type': 'application/x-www-form-urlencoded',
			...
			getGoodsCategoryList(){
				app.request().postRequest(
					...
					if (res.code === 1) {
						...
						if(res.result[0].id){
							this.firstcategoryid = res.result[0].id;
							this.categorytitle = res.result[0].Name;
							this.menuCurrentId = res.result[0].id;
						}
						this.getGoodsList();
					}
					else {
			...
			showForm(goods){
				...
				if(goodsCart){
					...
					if(currentGoods){
						...
						for (let i in goods.has_many_sku){
							if(goods.has_many_sku[i].id == currentGoods.has_many_sku.id){
								if(!selectStatus){
									goods.has_many_sku[i].select = true
									goods.number = currentGoods.number
									selectStatus = true
								}else{
									goods.has_many_sku[i].select = false
								}
							}else{
								goods.has_many_sku[i].select = false
							}
			...
			formAddGoodsChange(number,goodsData){
				console.log("加入购物车的数据1",goodsData);
				let that = this;
				if(number >= 1){
					for(let i in goodsData.has_many_sku){
						if(goodsData.has_many_sku[i].select == true ){
							that.currentGoodsData.has_many_sku = goodsData.has_many_sku[i]
							break
						}
					}
				}
				console.log("加入购物车的数据2",that.currentGoodsData);
			...
			selectGoodsForm(goods,formChild){
				...
				if(goodsCart){
					for(let i in goodsCart){
						if(goodsCart[i].id == goods.id && goodsCart[i].has_many_sku.id == formChild.id){
			...
			addGoodsChange(number,rowData){
				console.log("新增商品3",number,rowData);
				...
				if(carts.length != 0){
					...
					for(var i in shoppCart){
						if(shoppCart[i].id == rowData.id){
							if(isFormGoods){
								//相同商品比较规格是否也相同
								if(shoppCart[i].has_many_sku.id == rowData.has_many_sku.id){
									existedGoods = true;
								}
							}else{
								existedGoods = true;
							...
							if(existedGoods){
								...
								if(isFormGoods){
									shoppCart[i].Price = rowData.has_many_sku.Price
									shoppCart[i].MarketPrice = rowData.has_many_sku.CostPrice
									shoppCart[i].total = number *  rowData.has_many_sku.Price
								}else{
									shoppCart[i].Price = rowData.Price
									shoppCart[i].total = number *  rowData.Price
									shoppCart[i].MarketPrice = rowData.MarketPrice
								}
				...
				//第一次添加
				if(!existedGoods){
					console.log("是否是第一次添加",existedGoods);
					if(rowData.has_many_sku && rowData.has_many_sku[0]){
						rowData.Price = rowData.has_many_sku.Price;
						rowData.MarketPrice = rowData.has_many_sku.MarketPrice;
						rowData.total = number *  rowData.has_many_sku.Price
					}else{
				...
				console.log("加入购物车的总价",totalPrice);
				// return;

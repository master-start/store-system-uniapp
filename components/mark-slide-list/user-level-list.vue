			...
			<view class="slide-box" v-for="(item, index) in listData" :key="index">
				...
				>
					<view class="now-message-info" hover-class="uni-item--hover" :style="{ width: windowWidth + 'px' }" @click="clickItemMethod(item)">
						...
						<view class="icon-circle" v-else-if="item.LevelName">{{ item.LevelName }}</view>
						<view class="list-right">
							<view class="list-title" v-if="item.LevelName">
								{{ item.LevelName }}
								<block class="list-detail" v-if="item.Is_Hide == 0">（显示）</block>
								<block class="list-detail" v-if="item.Is_Hide == 1">（隐藏）</block>
							</view>
							<view class="list-detail" v-if="item.Description">{{item.Description}}</view>
						</view>
						<view class="list-right-1" v-if="item.Discount"> 折扣：{{item.Discount}}</view>
					</view>
					<view class="group-btn">
						<view class="btn-div" @click="clickMethod(item, '编辑', 0)" style="background:#0f80ff">编辑</view>
						<view class="btn-div" v-if="item.Is_Hide == 0" @click="clickMethod(item, '隐藏', 1)" style="background:#ff3b32">隐藏</view>
						<view class="btn-div" v-if="item.Is_Hide == 1" @click="clickMethod(item, '显示', 0)" style="background:#ff3b32">显示</view>
					</view>

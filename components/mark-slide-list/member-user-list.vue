			...
			<view class="slide-box" v-for="(item, index) in listData" :key="index">
				...
				>
					<view class="now-message-info" hover-class="uni-item--hover" :style="{ width: windowWidth + 'px' }" @click="clickItemMethod(item)">
						<image class="icon-image" v-if="item.UserImage" :src="item.UserImage"></image>
						<view class="icon-circle" v-else-if="item.Name">{{ item.Name }}</view>
						<view class="list-right">
							<view class="list-title" v-if="item.Name">
								{{ item.Name }}
								<block class="list-detail" v-if="item.Status == 0">（启用）</block>
								<block class="list-detail" v-if="item.Status == 1">（停用）</block>
							</view>
							<view class="list-detail" v-if="item.Remark">{{ item.Remark }} 消费：{{item.Amount}}元</view>
						</view>
						<view class="list-right-1" v-if="item.Mobile">{{ item.Mobile }}</view>
					</view>
					<view class="group-btn">
						<view class="btn-div" @click="clickMethod(item, '编辑', 0)" style="background:#0f80ff">编辑</view>
						<view class="btn-div" v-if="item.Status == 0" @click="clickMethod(item, '停用', 1)" style="background:#ff3b32">停用</view>
						<view class="btn-div" v-if="item.Status == 1" @click="clickMethod(item, '启用', 0)" style="background:#ff3b32">启用</view>
					</view>

...
<script>
export default {
	// https://github.com/Sunnshino/sunui-upimg
	...
	watch: {
		url(val) {
			this.upload_before_list.map((item) => {
				console.log("设置的换成数据为11",item);
				this.upload_cache_list.push(item);
			});
			this.emit();
		}
	},
	methods: {
		...
		uploadFile(paths) {
			...
			Promise.all(promises).then((data) => {
				...
				if(data && data[0] != undefined){
					...
					// console.log("111上传后返回的数据",result);
					// console.log("111上传后返回的数据222",data);
					this.upload_cache_list.push(...result);
					// console.log("111上传后的数据为",this.upload_cache_list);
				}
				this.emit();
			}).catch((res) => {
				console.log(res);
		...
		remove(idx) {
			...
			this.$emit('remove', this.upload_cache_list);
		},
		emit() {
